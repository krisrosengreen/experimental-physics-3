...
import helpers.cannloader as cann
import os
...
class AnalyzeMain:
    ...
    def plot_file(self, file):
        ...
        """
        xs, ys = self.loadfile(file)
        #data = mca.load_mca(file)
        #xs = np.arange(0, len(data), 1)
        name = file.split('/')[-1].split('.')[0]

        self.data_used["xs"] = xs
        self.data_used["ys"] = ys

        self.quick_plot(xs*self.xscale, ys*self.yscale, name)
    ...
    def fit_to_peak(self, xs, ys, index_peak):
        ...
        params, _ = curve_fit(gauss, xdata, ydata, p0=[
            min(ydata), max(ydata), xs[index_peak], 1])
        return params, (troughs_index, xdata, ydata)
    ...
    def plot_peaks_with_gauss(self, xs, ys, peaks, name):
        ...
        for c, peak in enumerate(peaks):
            params, (troughs, xdata, ydata) = self.fit_to_peak(xs, ys, peak)
            ...
            print(f"At peak: {peak}", f"x_top is: {params[2]:.5f}",
                  f"y_top is: {ytop:.5f}", f"energy: {params[2]*self.xscale} keV")
...
class AnalyzeEX2(AnalyzeMain):
    """
    class used to analyze data gathered in experiment 2
    """
    def __init__(self, kwargs):
    ...
    def compare_peaks(self, file1, file2, diff=5):
        ...
        _,file1_data = self.loadfile(file1)
        _,file2_data = self.loadfile(file2)
...
class AnalyzeEX1(AnalyzeMain):
    """
    class used to analyze data gathered in experiment 1
    """
    def __init__(self, kwargs):
        super().__init__(kwargs)

    def loadfile(self, filename):
        xs, ys, _ = cann.load_file(filename)
        return xs, ys
